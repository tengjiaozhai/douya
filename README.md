...
1.  **短期上下文管理**: 自动维护最近 10 条消息的活跃上下文，确保响应速度与成本平衡。
2.  **持久化归档**: 超出阈值的历史消息自动存入持久化存储（`DatabaseStore`），支持全量历史追溯。
3.  **异步总结向量化**:
    - **智能总结**: 自动提取并生成归档消息段的精简摘要。
    - **向量化知识库**: 使用 `UserVectorApp` 将摘要存入 Chroma 向量库。
    - **无感执行**: 基于 Java 21 虚拟线程在后台异步完成，不影响用户对话的实时性。
    - **价值**: 将杂乱的聊天记录转化为结构化的用户信息知识库，为后续 RAG 检索提供精准支持。
