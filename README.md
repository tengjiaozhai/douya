...
- **规范模型请求参数**: 将豆包模型的 `model` 参数修正为方舟平台的“推理接入点 ID (Endpoint ID)”，解决了非法模型名称导致的后端报错。
- **重构视觉感知提示词**: 将视觉理解模型（Vision Model）的定位从专一的“大厨助手”重构为通用的“视觉分析专家”。通过精细化的结构化指令（核心主体、细节特征、环境氛围、动作状态），使其能够泛化处理任何类型的视觉素材，并为下游的业务逻辑提供客观、高保真的事实基础。
- **视觉 Token 效率优化**: 引入了“零冗余约束”与“键值对输出”模式，禁止模型产生任何“描述性废话”，显著降低了视觉感知环节的 Token 开销。
- **首创“意图先行”图片交互流程**: 放弃了收到图片即自动解析的旧模式，改为多轮交互：收到图片后先通过 `Store` 暂存路径并主动询问用户意图，待用户回复指令后再结合图片进行针对性解析。此举不仅大幅增强了拟人化体验，更有效避免了无目的解析带来的资源浪费。
