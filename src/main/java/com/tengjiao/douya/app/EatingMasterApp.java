    ...
    private String process(UserMessage userMessage, String userId) {
        ...
        try {
            ...
            if (invoke.isPresent()){
                ...
                overAllState.value("EatingMaster").ifPresent(outPut -> {
                    switch (outPut){
                        case AssistantMessage message -> {
                            log.info("[Multi-Agent] 获取到 EatingMaster 输出: {}", message.getText());
                        ...
                        default -> log.info("[Multi-Agent] 获取到 EatingMaster 输出: {}", outPut);
                    }
                });
                overAllState.value("VisionUnderstand").ifPresent(outPut -> {
                    switch (outPut){
                        case AssistantMessage message -> {
                            log.info("[Multi-Agent] 获取到 EatingMaster 输出: {}", message.getText());
                            Map<String, Object> data = new HashMap<>();
                ...
                log.info("[Multi-Agent] 处理完成: {}", overAllState.data());
                return overAllState.data().toString();
            }
            return "抱歉，我的大脑暂时断片了";
        } catch (Exception e) {
